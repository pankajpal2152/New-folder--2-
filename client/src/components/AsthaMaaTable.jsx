...
const AsthaMaaModal = ({ member, mode, onClose, onSuccess }) => {
  ...
  const cleanInitialImage =
    extractBase64(member.AsthaMaProfileImage) || DUMMY_AVATAR;
  ...
  const {
    ...
    defaultValues: {
      ...
      state: null,
      district: null,
      city: member.AsthaMaCity || "",
      block: member.AsthaMaBlockName || "",
      postOffice: member.AsthaMaPO || "",
      policeStation: member.AsthaMaPS || "",
      gramPanchayet: member.AsthaMaGramPanchayet || "",
      village: member.AsthaMaVillage || "",
      pinCode: String(member.AsthaMaPincode || ""),
      mobileNo: member.AsthaMaContactNo || "",
      email: member.AsthaMaMailId || "",
      userName: member.AsthaMaSignupUserName || "",
      password: member.AsthaMaSignupPassword || "",
      bankName: member.AsthaMaBankName || "",
      branchName: member.AsthaMaBranchName || "",
      accountNo: member.AsthaMaBankAcctNo || "",
      ifsCode: member.AsthaMaIFSCode || "",
      panNo: member.AsthaMaPanNo || "",
      ...
      .then((data) => {
        ...
        if (member.AsthaMaStateName) {
          const matchedState = formattedStates.find(
            (s) => s.label === member.AsthaMaStateName,
          );
        ...
        .then((data) => {
          ...
          if (member.AsthaMaDistName) {
            const matchedDist = formattedDistricts.find(
              (d) => d.label === member.AsthaMaDistName,
            );
  ...
  const onSubmit = async (data) => {
    ...
    if (dbPayload.AsthaMaDOB)
      dbPayload.AsthaMaDOB = String(dbPayload.AsthaMaDOB).substring(0, 10);

    try {
      toast.loading("Updating Astha Maa...", { toastId: "update" });
      const res = await fetch(
        `${API_BASE_URL}/asthamaa/${member.AsthaMaRegId}`,
        {
          method: "PUT",
      ...
      if (res.ok) {
        toast.success("Astha Maa updated successfully!", {
          position: "top-right",
        });
        onSuccess();
      } else {
        toast.error("Failed to update. Check backend logs.", {
          position: "top-right",
        });
      }
    } catch (error) {
    ...
    <div style={styles.modalOverlay}>
      <div style={{ ...styles.modalContent, maxWidth: "1000px", padding: "0" }}>
        <div style={styles.cardHeader}>
          <h5 style={{ margin: 0 }}>
            {isView ? "View" : "Edit"} Astha Maa Details
          </h5>
          ...
          <div style={styles.profileSection}>
            ...
            <div>
              <p style={styles.hintText}>
                ...
                {Number(member.AsthaMaIsActive) === 2 ? "Approved" : "Pending"}
              </p>
              {Number(member.AsthaMaIsActive) === 2 &&
                member.AsthaMaAprovedBy && (
                  <>
                    <p style={styles.hintText}>
                      <strong>Approved By:</strong>{" "}
                      {member.ApproverDisplayName || member.AsthaMaAprovedBy}
                    </p>
                    <p style={styles.hintText}>
                      <strong>Approval Date:</strong>{" "}
                      {formatDisplayDate(member.AsthaMaAprovalDate)}
                    </p>
                  </>
                )}
              {!isView && (
...
const AsthaMaaTable = ({ refreshTrigger, externalFilters }) => {
        ...
        if (user.role === "Astha Didi") {
          data = data.filter(
            (member) =>
              ...
              String(user.id || user.UserSignUpId),
          );
        } else if (user.role === "Astha Maa") {
          data = data.filter(
            (member) =>
              String(member.AsthaMaRegId) === String(user.ProfileRegId),
          );
        }
      }
      setMembers(data);
    } catch (error) {
      toast.error("Failed to load table data.", { position: "top-right" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((member) => {
      ...
      if (externalFilters?.filterState) {
        const dbState = member.AsthaMaStateName
          ? String(member.AsthaMaStateName).trim().toLowerCase()
          : "";
    ...
    if (type === "approve") {
      ...
      try {
        ...
        const stateObj = states.find(
          (s) => s.StateName === member.AsthaMaStateName,
        );
        ...
        if (stateObj) {
          ...
          const distObj = dists.find(
            (d) => d.DistName === member.AsthaMaDistName,
          );
        ...
        console.error(
          "Error fetching state/dist IDs for approval generation:",
          e,
        );
      }
  ...
  const confirmDelete = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/asthamaa/${selectedRow.AsthaMaRegId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        setMembers((prev) =>
          prev.filter((m) => m.AsthaMaRegId !== selectedRow.AsthaMaRegId),
        );
        closeModal();
      } else {
                  ...
                  {currentMembers.map((row) => (
                    <tr key={row.AsthaMaRegId}>
                      <td style={styles.stickyLeftTd}>
                        <img
                          src={
                            extractBase64(row.AsthaMaProfileImage) ||
                            DUMMY_AVATAR
                          }
                          alt="User"
                          style={{
                      ...
                      <td style={styles.td}>
                        {formatDisplayDate(row.AsthaMaDOB)}
                      </td>
                      ...
                      <td style={styles.td}>
                        {row.ApproverDisplayName || row.AsthaMaAprovedBy || "-"}
                      </td>
                      <td style={styles.td}>
                        {formatDisplayDate(row.AsthaMaAprovalDate)}
                      </td>
      ...
      {deleteModal && selectedRow && (
        <div style={styles.modalOverlay}>
          ...
          >
            ...
            <p>
              Delete <strong>{selectedRow.AsthaMaUserName}</strong>?
            </p>
            <div
          ...
          >
            ...
            >
              <p style={{ margin: "6px 0" }}>
                <strong>Candidate Name:</strong> {selectedRow.AsthaMaUserName}
              </p>
              <p style={{ margin: "6px 0" }}>
              ...
              <p style={{ margin: "6px 0" }}>
                <strong>Approval Date:</strong>{" "}
                {approvalData.dbDate || "Loading..."}
              </p>
              <p style={{ margin: "6px 0" }}>
                <strong>Authorized Approver:</strong> {userName}
              </p>
            </div>

            <p style={{ marginBottom: "20px", color: "#697a8d" }}>
              Do you want to confirm this approval and store this data?
            </p>
