...
const SupervisorModal = ({ member, mode, onClose, onSuccess }) => {
  ...
  const cleanInitialImage =
    extractBase64(member.SupProfileImage) || DUMMY_AVATAR;
      ...
      try {
        ...
        if (member.SupStateName) {
          const matchedState = formattedStates.find(
            (s) => s.label.trim() === member.SupStateName.trim(),
          );
          if (matchedState) {
            ...
            if (member.SupDistName) {
              const matchedDist = formattedDistricts.find(
                (d) => d.label.trim() === member.SupDistName.trim(),
              );
              if (matchedDist) {
  ...
  const onSubmit = async (data) => {
    ...
    if (dbPayload.SupDOB)
      dbPayload.SupDOB = String(dbPayload.SupDOB).substring(0, 10);
    ...
    <div style={styles.modalOverlay}>
      <div style={{ ...styles.modalContent, maxWidth: "1000px", padding: "0" }}>
        <div style={styles.cardHeader}>
          <h5 style={{ margin: 0 }}>
            {isView ? "View" : "Edit"} Supervisor Details
          </h5>
          ...
          <div style={styles.profileSection}>
            ...
            <div>
              <p style={styles.hintText}>
                ...
                {Number(member.SupIsActive) === 2 ? "Approved" : "Pending"}
              </p>
              {Number(member.SupIsActive) === 2 && member.SupAprovedBy && (
                <>
                  <p style={styles.hintText}>
                    <strong>Approved By:</strong>{" "}
                    {member.ApproverDisplayName || member.SupAprovedBy}
                  </p>
                  <p style={styles.hintText}>
                    <strong>Approval Date:</strong>{" "}
                    {formatDisplayDate(member.SupAprovedDate)}
                  </p>
                  <p style={styles.hintText}>
                    <strong>Approval ID:</strong> {member.SupRegNo || "-"}
                  </p>
                </>
              )}
              {!isView && (
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter((member) => {
      ...
      if (externalFilters?.filterState) {
        const dbState = member.SupStateName
          ? String(member.SupStateName).trim().toLowerCase()
          : "";
    ...
    if (type === "approve") {
      ...
      try {
        ...
        const stateObj = states.find(
          (s) => s.StateName === member.SupStateName,
        );
  ...
  const confirmDelete = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/supervisor/${selectedRow.SupRegId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        setMembers((prev) =>
          prev.filter((m) => m.SupRegId !== selectedRow.SupRegId),
        );
        closeModal();
      } else {
                  ...
                  {currentMembers.map((row) => (
                    <tr key={row.SupRegId}>
                      <td style={styles.stickyLeftTd}>
                        <img
                          src={
                            extractBase64(row.SupProfileImage) || DUMMY_AVATAR
                          }
                          alt="User"
                          style={{
                            width: "40px",
                      ...
                      <td style={styles.td}>
                        {row.SupSignupEmail || row.SupMailId || "-"}
                      </td>
                      ...
                      <td
                        style={{
                          ...styles.td,
                          color:
                            Number(row.SupIsActive) === 2 ? "green" : "orange",
                          fontWeight: "bold",
                        }}
                      >
                        {Number(row.SupIsActive) === 2 ? "Approved" : "Pending"}
                      </td>
                      <td style={styles.td}>
                        {row.ApproverDisplayName || row.SupAprovedBy || "-"}
                      </td>
                      <td style={styles.td}>
                        {formatDisplayDate(row.SupAprovedDate)}
                      </td>
      ...
      {deleteModal && selectedRow && (
        <div style={styles.modalOverlay}>
          ...
          >
            ...
            <p>
              Delete <strong>{selectedRow.SupName}</strong>?
            </p>
            <div
          ...
          >
            ...
            >
              <p style={{ margin: "6px 0" }}>
                <strong>Candidate Name:</strong> {selectedRow.SupName}
              </p>
              <p style={{ margin: "6px 0" }}>
