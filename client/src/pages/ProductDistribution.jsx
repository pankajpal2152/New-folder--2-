...
export default function ProductDistribution() {
  ...
  // ✅ Checkbox State for TrnType Filtering (Default to ALL)
  ...
  const filteredHistory = useMemo(() => {
    return history.filter((h) => {
      ...
      if (searchTerm) {
        const lowerSearch = searchTerm.toLowerCase();
        const combinedString =
          `${h.TransactionDate} ${h.SenderHead} ${h.SenderAcctName} ${h.ProductName} ${h.Qty} ${h.SenderMode} ${h.Remarks} ${h.ReceiverInfo}`.toLowerCase();
        searchMatch = combinedString.includes(lowerSearch);
      }

      // ✅ Handle the Checkbox Filter Logic
      let typeMatch = true;
      if (
        trnFilter === "TRANSFER" &&
