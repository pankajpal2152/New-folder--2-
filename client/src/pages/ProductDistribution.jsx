...
const ProductDistribution = () => {
    const [formData, setFormData] = useState({ 
        AcctHeadId: '', ReceiverId: '', SupervisorId: '', ProductName: '', DistributedQty: '', Remarks: 'By Transfer', Date: new Date().toISOString().split('T')[0] 
    });
        ...
        try {
            ...
            if (user.UserSignUpRole === 'State Super Administrator') {
                ...
                if (dnHead) {
                    ...
                    setFilteredAccounts(acctTableRes.data.filter(acc => acc.AcctHead === 'DN'));
                }
            }
    ...
    const fetchSupervisors = async (distId) => {
        try {
            const res = await axios.get(`${API_BASE_URL}/supervisors-by-dist/${distId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({ AcctHeadId: defaultHead, ReceiverId: '', SupervisorId: '', ProductName: '', DistributedQty: '', Remarks: 'By Transfer', Date: new Date().toISOString().split('T')[0] });
    ...
    const selectedSupervisorHead = formData.SupervisorId ? supervisors.find(s => String(s.id) === String(formData.SupervisorId))?.Head : '';
    const selectedSupervisorName = supervisors.find(s => String(s.id) === String(formData.SupervisorId))?.name || '';
                ...
                <div style={styles.leftPanel}>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="d-flex align-items-center mt-1 px-1 gap-2">
                            ...
                            <select style={{...styles.input, width: '120px'}} value={formData.AcctHeadId} onChange={(e) => setFormData({...formData, AcctHeadId: e.target.value})} disabled={user.UserSignUpRole === 'State Super Administrator'}>
                                <option value=""></option>
                        ...
                        <div className="d-flex align-items-center mt-2 px-1 gap-2 mb-2">
                            <label style={{...styles.label, width: '90px'}}>Supervisor</label>
                            <select style={{...styles.input, width: '250px'}} value={formData.SupervisorId} onChange={(e) => setFormData({...formData, SupervisorId: e.target.value})}>
                                <option value="">-- Select Supervisor --</option>
                                {supervisors.map(s => <option key={s.id} value={s.id}>{s.id} - {s.name}</option>)}
                            </select>
                            {/* Dynamic Text Display */}
                            <span style={{...styles.redText, width: '50px', marginLeft: '10px'}}>{selectedSupervisorHead}</span>
                            <span style={{...styles.redText}}>{selectedSupervisorName}</span>
                        </div>
                            ...
                            <table className="table table-sm mb-0" style={{ fontSize: '11px', borderCollapse: 'collapse' }}>
                                <thead style={{ backgroundColor: '#f0f4f8', position: 'sticky', top: 0, color: '#005bb5' }}>
                                    <tr>
                                        ...
                                        <th style={{border: '1px solid #ccc', padding: '2px 4px'}}>Supervisor</th>
                                ...
                                <tbody>
                                    {history.map(row => (
                                        <tr key={row.DistId}>
                                            ...
                                            <td style={{border: '1px solid #ccc', padding: '2px 4px'}}>{row.SupervisorId}</td>
